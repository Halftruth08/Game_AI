...
def build_appendix(targets, filena='PTsynonyms01.txt',
                   model={}, mode='w', min_votes1=5, related=False):
...
#    tr.close()
    ...
    branch_syns = []
    for word in missed:
        newwords = scrape_power_thes(
            word,
            pass_on_words=True,
            write_here=False,
            min_upvotes=min_votes1)
    ...
    for i in range(iterations):
        ...
        for target in targets:
            try:
                ...
                temp = []
            temp1 = list(map(lambda a: a.replace(
                ' (page does not exist)', ''), temp))
...
def scrape_wiktionary(word, require_success=0):
    ...
    def follow_breadcrumbs(pocket):
        ...
        if len(bullets) > 1 or len(bullets[0].getchildren(
        )) > 1:  # if this is one, maybe do something special
            ...
            if len(bullet_content[0].xpath('span/i/a')) > 0:
                ...
                if len(new_words) == 0:
                    if require_success == 1:
                        raise IOError(
                            "no existing strategies worked for %s" %
                            word)
                else:
            ...
            if len(bullets) > 1 or len(bullets[0].getchildren(
                ...
                if len(bullet_content[0].xpath('span/i/a')) > 0:
                    ...
                    if len(new_words) == 0:
                        if require_success == 1:
                            raise IOError(
                                "no existing strategies worked for %s" % word)
                    else:
                        entry_words.extend(new_words)
    return entry_words
...
def scrape_power_thes(word, filen="appendix01.txt", min_upvotes=5,
    ...
    while exitc == 0:
        ...
        for entry in entries:
            v, w = entry.xpath('td')
            if int(
                    v.xpath('div/div[@class="rating"]/text()')[0]) < min_upvotes:
                exitc = 1
                # print(word)
                break
...
def prep_raw(inp, out, remove_and=1, dev=0, enc=ENC):
    ...
    # for i in range(5000): #500 is the number of sentences to use during
    # development
    lines = 0
...
def linewise_filter(line, rem_and=1, lower=1):
    ...
    line = line.translate(str.maketrans(
        {key: None for key in string.punctuation or string.digits}))
    if rem_and == 1:
...
"""
    ...
    for i in targs.keys():
        ...
        m = np.max(odds)
        n = nlevels
        wnum = list(map(lambda a: int(np.floor(a)),
                        np.multiply(odds, (n + 1.) / m)))
...
def targetlist_update2(bigrams, targs, nlevels):
    ...
    for i2 in bigrams.keys():
        bg3[i2] = {}
        mincut = max([item for sublist in list(bigrams[i2].values())
                      for item in sublist]) // (nlevels + 1)
        for i3 in bigrams[i2].keys():
            if max(bigrams[i2][i3]) > mincut:
                bg3[i2][i3] = bigrams[i2][i3]
    for i2 in bg3.keys():
