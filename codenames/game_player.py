...
def codemaster(model):
    ...
    def get_clues_array(targets, blue, grey, black):
        ...
        for candidate in candidates:
            for i2 in range(len(targets)):
                clue_likelihoods['out'][candidate][i2] += coloc[targets[i2]
                                                                ].get(candidate, 0.)
                clue_likelihoods['in'][candidate][i2] += coloc[candidate].get(
                    targets[i2], 0.)
            temp = len(targets)
            for i2 in range(len(blue)):
                clue_likelihoods['out'][candidate][i2 +
                                                   temp] += coloc[blue[i2]].get(candidate, 0.)
                clue_likelihoods['in'][candidate][i2 +
                                                  temp] += coloc[candidate].get(blue[i2], 0.)
            temp += len(blue)
            for i2 in range(len(grey)):
                clue_likelihoods['out'][candidate][i2 +
                                                   temp] += coloc[grey[i2]].get(candidate, 0.)
                clue_likelihoods['in'][candidate][i2 +
                                                  temp] += coloc[candidate].get(grey[i2], 0.)
            temp += len(grey)
            for i2 in range(len(black)):
                clue_likelihoods['out'][candidate][i2 +
                                                   temp] += coloc[black[i2]].get(candidate, 0.)
                clue_likelihoods['in'][candidate][i2 +
                                                  temp] += coloc[candidate].get(black[i2], 0.)
    ...
    def filter_out_cheat_clues(candidates):
        honest = []
    ...
    def play_turn():
        ...
        for key in weights.keys():
            all_in[key] = list(filter(lambda a: all(
                [a.find(key) > -1, a.find('in') > -1]), likes.columns))
            all_out[key] = list(filter(lambda a: all(
                [a.find(key) > -1, a.find('out') > -1]), likes.columns))
        likes['all_i_sum'] = 0.
        likes['all_o_sum'] = 0.
        for key in weights.keys():
            temp1 = key + 's_i_sum'
        ...
        n_remaining = float(sum([sum(remaining[key])
                                 for key in weights.keys()]))
        temp = list(filter(lambda a: all(
            [a.find('red') > -1, a.find('in') > -1]), cols))
        top_x = []
        top_i = []
        top_o = []
        top_io = []
        ...
        for i in likes.index:
            temp2 = likes.loc[i, temp[0]:temp[len(
                temp) - 1]].nlargest(n=sum(remaining['red']))
            if sum(temp2) == 0:
                newdex.remove(i)
            ...
            temp3 = list(map(lambda a: a.replace(
                'in', 'out'), temp2.index.tolist()))
            temp4 = []
            temp5 = []
            temp6 = []
        ...
        for i in likes.index:
            ...
            if temp1 > 0.:
                for i2 in range(len(templist)):
                    ...
                    temp0 = l2 * \
                        (itotl * temp1 -
                         templist[i2] * weights['red']) / (itotl - templist[i2])
                    ...
                    for i3 in range(len(templist)):
                        if not i2 == i3:
                            l3 = l2 * templist[i3] / \
                                (1. - templist[i2] / itotl)
                            l3i = l2i * \
                                (templist[i3] * itotl / (itotl - templist[i2])
                                 * (1. - ign) + ign / (n_remaining - 1.))
                            ...
                            for i4 in range(len(templist)):
                                if not i4 == i2 and not i4 == i3:
                                    l4 = l3 * \
                                        templist[i4] * (itotl / (itotl -
                                                                 (templist[i2] + templist[i3])))
                                    l4i = l3i * (templist[i4] * itotl / (itotl - (templist[i2] + templist[i3])) * (
                                        1. - ign) + ign / (n_remaining - 2.))
