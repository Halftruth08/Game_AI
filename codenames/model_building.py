...
def store_model(model):
    ...
    existing_models = list(
        filter(
            lambda a: a.find('model') > -1,
            os.listdir(MODELS)))
    model_numbers = list(
        map(lambda a: int(a.replace('model', '')), existing_models))
    ...
    for wordentry in wordlist:
        ...
        if len(wordentry) == 1:
            for rep in range(wnum):
                ...
                # leaving in part of speech (noun) for now
                vocabulary.extend(wordentry[0])
            temp = [word]
...
(noun)|shrub|bush
    ...
    try:
        while True:
            word, num = th.readline().replace('\n', '').split(
                '|')  # num is the number of senses for a word
            num = int(num) * weight
            entry = [word, str(num)]
    ...
    # data is stored as [["abel janszoon tasman",["(noun)","tasman","abel
    # tasman","abel janszoon tasman","navigator"]]...]
    return data
